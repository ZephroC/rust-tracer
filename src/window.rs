...
pub fn run(res:Resolution, scene:&SceneState, samples:u8) -> GameResult {
    let mut state =  WindowState::new(res, scene, samples);
...
impl WindowState<'_> {
    fn new(res: Resolution, scene: &SceneState, samples:u8) -> WindowState {
        let byte_stride = 4;
...
impl event::EventHandler for WindowState<'_> {
    ...
    fn draw(&mut self, _ctx: &mut Context) -> GameResult {
        ...
        self.scene.rasterise(&mut self.frame, &self.res, self.samples);
        let image =
